               ...
               if($this->config->get('module_lts_vendor_status')) {
                     ...
                     foreach($vendor_products as $order_product_id) {
                         ...
                         if($order_product_info) {
                             ...
                             if(!empty($product_info['vendor_id'])) {
                                 $this->db->query("INSERT INTO " . DB_PREFIX . "lts_order_product SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', vendor_id = '" . (int)$product_info['vendor_id'] . "', product_id = '" . (int)$order_product_info['product_id'] . "', customer_id = '" . (int)$data['customer_id'] . "', quantity = '" . (int)$order_product_info['quantity'] . "', price = '" . (float)$order_product_info['price'] . "', total = '" . (float)$order_product_info['total'] . "', tax = '" . (float)$order_product_info['tax'] . "', date_added = NOW(), date_modified = NOW()");
			...
			// Update the DB with the new statuses
                ...
                if($this->config->get('module_lts_vendor_status')) {
                    ...
                    $vendor_products = $this->model_account_vendor_lts_order->getVendorOrderProducts($order_id);
                        ...
                        $admin_get_mail_by_vendor_status_change=$this->config->get('module_lts_vendor_order_status_change_by_vendor');                        
                        if($admin_get_mail_by_vendor_status_change)
                        {
                            $this->load->language('account/vendor/lts_order_alert');
                            $data['text_received'] = $this->language->get('text_received_admin');
                                      $data['text_order_id'] = $this->language->get('text_order_id');
                                      $data['text_date_added'] = $this->language->get('text_date_added');
                                      $data['text_order_status'] = $this->language->get('text_order_status');
                                      $data['text_product'] = $this->language->get('text_product');
                                      ...
                                      $data['date_added'] = date($this->language->get('date_format_short'), strtotime($vendor_product['date_added']));
                                      $order_product = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_product_id = '". (int)$vendor_product['order_product_id'] ."' AND order_id = '" . (int)$order_id . "'")->row;                                
                                          $data['name']     = $order_product['name'];
                                          $data['model']    = $order_product['model'];
                                          $data['quantity'] = $order_product['quantity'];
                                          $data['total']    = $order_product['total'];
                                      $order_status_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$vendor_product['order_status_id'] . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
                                ...
                                $mail->setTo($this->config->get('config_email'));
                                      $mail->setFrom($vendor_info['email']);
                                      $mail->setSender(html_entity_decode($vendor_info['store_name'], ENT_QUOTES, 'UTF-8'));
                                      $mail->setSubject(html_entity_decode(sprintf($this->language->get('text_subject'), $vendor_info['store_owner'], $order_id), ENT_QUOTES, 'UTF-8'));
                                      $mail->setText($this->load->view('account/vendor/lts_order_alert', $data));
                                      $mail->send();
                        }
                        
                        $is_subscription_vendor=$this->model_account_vendor_lts_subscription->isPlanNotExpired($vendor_product['vendor_id']); 
                        ...
                        if($vendor_info) {
                            ...
                            {
                               $vendor_commission_info = $this->db->query("SELECT * FROM ". DB_PREFIX ."lts_commission WHERE order_id = '" . (int)$order_id . "' AND product_id = '" . (int)$vendor_product['product_id'] . "' AND vendor_id = '" . (int)$vendor_product['vendor_id'] . "' AND type = 'sale'")->row;
                                if(!$vendor_commission_info)
                                {
                                    ...
                                    if($vendor_info['commission_rate'] != 0) {
                                      ...
                                      if($this->config->get('module_lts_vendor_default_commission'))
                                        {
                                            $vendor_commission_rate = $this->config->get('module_lts_vendor_default_commission');
                                        }
                                        else{
                                        $vendor_commission_rate=50;
                                        }
                                    }   

                                    if($is_subscription_vendor)
                                    {

                                        $amount_rate = ($vendor_product['total'] * $is_subscription_vendor['subscription_commission']) / 100;
                                    ...
                                    $default_commission=1;
                                     ...
                                     $is_subscription_vendor_info=$this->db->query("SELECT * FROM ". DB_PREFIX ."lts_plan WHERE vendor_id = '". (int)$vendor_product['vendor_id'] ."'")->row;

                                        if($is_subscription_vendor_info)
                                        {                                    
                                            $this->model_account_vendor_lts_subscription->deletePlan($vendor_product['vendor_id']);
                                        }

                                    }                          
                     
                                    
                                    $this->db->query("INSERT INTO " . DB_PREFIX . "lts_commission SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$vendor_product['order_product_id'] . "', product_id = '" . (int)$vendor_product['product_id'] . "', vendor_id = '" . (int)$vendor_product['vendor_id'] . "', amount = '" . (float)$amount_rate . "', type = 'sale', status = '1', date_added = NOW(), date_modified = NOW(), default_commission='".$default_commission."'");
