...
class ModelAccountVendorLtsCustomer extends Model {
	...
	public function get_custom_id($vendorid='')
	{
		$query=$this->db->query("SELECT  customer_id FROM " . DB_PREFIX . "lts_order_product WHERE vendor_id ='".$vendorid."' GROUP BY customer_id");
		return $query->rows;
	}
	public function getCustomers($data = array(), $customer_id='') {
		 

		 $sql ="SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "lts_order_product op ON (c.customer_id = op.customer_id) JOIN " . DB_PREFIX . "customer_group_description cgd ";		 
		 
		$sql .= "WHERE op.customer_id='".$customer_id."' AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function filterCustomers($data = array(), $customer_id='') {
		
		$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) JOIN " . DB_PREFIX . " lts_order_product ON (c.customer_group_id = '".$customer_id."')";
		
		 		
		
		$sql .= " cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getTotalCustomers($data = array() , $customer_id='') {
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE customer_id='".$customer_id."'";
        ...
        public function getCustomerid($vendorid='')
        {
        	$query=$this->db->query("SELECT customer_id FROM " . DB_PREFIX . "lts_order_product WHERE vendor_id='".$vendorid."' GROUP BY customer_id");
        	return $query->num_rows;
        }

        public function getCustomerResult($vendorid='')
        {
	...
	public function getTotalLtsCustomer($vendorid)
	{
		$query = $this->db->query("SELECT COUNT(*) FROM " . DB_PREFIX . "lts_order_product WHERE vendor_id = '".$vendorid."'");
