...
class ModelAccountVendorLtsVendor extends Model {
   ...
   public function getVendorStatus($customer_id) {
	...
	public function isVendor($customer_id){
 		     ...
 		     $query = $this->db->query("SELECT vendor_id FROM `" . DB_PREFIX . "lts_vendor` WHERE  customer_id = '" . $customer_id . "'");   
        	  
		  return $query->row?$query->row['vendor_id']:0;
	}

	public function get_vendor_name($customer_id='')
	{
		 
		$result = $this->db->query("SELECT firstname, lastname FROM " . DB_PREFIX . "customer WHERE customer_id='".$customer_id."'");
		return $result->row;
	}

	public function isVendorSubscription($vendor_id='')
	{
		$result = $this->db->query("SELECT * FROM " . DB_PREFIX . "lts_plan WHERE vendor_id='".$vendor_id."'")->row;
		if($result)
		{
			
			if(date('Y-m-d h:i:s')<=$result['date_expire'])
			  {
			     return;		   
			  }
			  else
			  {		  	
			  	$this->db->query("DELETE FROM " . DB_PREFIX . "lts_plan WHERE vendor_id ='".$vendor_id."' AND subscription_id='".$result['subscription_id']."'");
			  }
		}
	} 
}